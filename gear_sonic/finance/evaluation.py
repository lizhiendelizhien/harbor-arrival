...
@torch.no_grad()
def evaluate_sequences(actor, sequences, *, num_envs=16, reward_contract=None):
    ...
    return {
        "reward_contract": deepcopy(reward_contract), "action_mode": "deterministic_mean",
...
def evaluate_checkpoint(checkpoint_path, *, device="cpu", num_envs=16, threads=2,
    ...
    try:
        ...
        actor.to(device)
        result = evaluate_sequences(actor, sequences, num_envs=num_envs, reward_contract=reward_contract)
        scope = ("custom_reference_pool" if custom_reference else
                 "legacy_reference_unverified" if legacy else "training_reference_pool")
        return {
            "schema_version": 1, "mode": "privileged_reference_tracking", "dataset_partition": "none",
            ...
            "num_envs": num_envs, "threads": threads, **result,
        }
    finally:
