...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            obs_dim_dict: Mapping from observation keys to their dimensions.
                Defaults to ``env_config.obs_dim_dict``.
            ...
            backbone_kwargs: Extra keyword arguments forwarded to the backbone
                constructor.
            action_dim: Number of action dimensions used by the policy.
        """
        super().__init__()
        ...
        if obs_dim_dict is None:
            obs_dim_dict = getattr(env_config, "obs_dim_dict", {})
        ...
        if self.num_actions is None:
            self.num_actions = getattr(env_config, "action_dim", None)
        if self.num_actions is None:
            raise ValueError("action_dim is required to construct Actor")
    ...
    def reset(self, dones=None):
        if hasattr(self.actor_module, "reset_cache"):
            self.actor_module.reset_cache(dones)

    def get_rollout_cache_state(self):
        if hasattr(self.actor_module, "get_cache_state"):
            return self.actor_module.get_cache_state()
        return None

    def _normalize_obs_dict(self, obs_dict):
    ...
    def forward(self, obs_dict, is_training=False, **kwargs):
        ...
        """
        obs_dict = self._normalize_obs_dict(obs_dict)
    ...
    def update_distribution(
        ...
        **kwargs,
        ...
        if last_step_only:
            mean = mean[:, -1]
        elif loss_start_index:
            mean = mean[:, loss_start_index:]
    ...
    def rollout(self, obs_dict, episode_attnmask=None, cur_dones=None, **kwargs):
        ...
        use_cache_rollout = hasattr(self.actor_module, "rollout_step") and (
            cur_dones is not None or episode_attnmask is None
        )
        if use_cache_rollout:
            step_obs_dict = self._normalize_obs_dict(obs_dict)
            mean = self.actor_module.rollout_step(
                step_obs_dict,
    ...
    def act_inference(self, obs_dict, episode_attnmask=None, cur_dones=None, **kwargs):
        ...
        if use_cache_rollout:
            step_obs_dict = self._normalize_obs_dict(obs_dict)
            actions_mean = self.actor_module.rollout_step(
                step_obs_dict,
                reset_mask=cur_dones,
                **kwargs,
            )
            if actions_mean.ndim == 3:
    ...
    def _rebuild_rollout_cache(self):
        if not (
            hasattr(self.actor_module, "rollout_step")
            and hasattr(self.actor_module, "reset_cache")
            and len(self.obs_dict_buffer.keys()) > 0
        ):
            return
        first_key = next(iter(self.obs_dict_buffer.keys()))
        history_len = self.obs_dict_buffer[first_key].shape[1]
        if history_len == 0:
            return

        self.actor_module.reset_cache()
        with torch.no_grad():
            for t in range(history_len):
                step_obs_dict = {
                    key: value[:, t] for key, value in self.obs_dict_buffer.items()
                }
                step_obs_dict = self._normalize_obs_dict(step_obs_dict)
                reset_mask = None
                if self.dones_buffer is not None and t > 0 and self.dones_buffer.shape[1] >= t:
                    reset_mask = self.dones_buffer[:, t - 1]
                self.actor_module.rollout_step(step_obs_dict, reset_mask=reset_mask)
    ...
    def init_rollout(self, preserve_history=False):
    ...
    def clear_rollout(self, preserve_history=False):
        """Clear the observation buffer after rollout phase."""
        if preserve_history:
            for key in list(self.obs_dict_buffer.keys()):
                self.obs_dict_buffer[key] = self.obs_dict_buffer[key][
                    :, -self.max_rollout_history :
                ].detach()
            if self.dones_buffer is not None:
                self.dones_buffer = self.dones_buffer[
                    :, -max(0, self.max_rollout_history - 1) :
                ].detach()
        else:
            self.obs_dict_buffer = TensorDict()
            self.dones_buffer = None
