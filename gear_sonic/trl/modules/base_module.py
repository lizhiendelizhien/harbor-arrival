...
class BaseModule(nn.Module):
    ...
    Args:
        obs_dim_dict: Mapping of observation name to its flat dimension.
            Falls back to ``env_config.obs_dim_dict`` if None.
        ...
        env_config: Optional environment configuration providing generic dimensions.
        algo_config: Algorithm configuration (stored but not used directly).
        process_output_dim: If True, replace ``"action_dim"`` sentinel values in
            ``output_dim`` with the actual action dimension.
        action_dim: Optional action dimension used for the ``"action_dim"`` sentinel.
    ...
    def __init__(
        ...
        if obs_dim_dict is None:
            self.obs_dim_dict = getattr(env_config, "obs_dim_dict", {}) if env_config else {}
        else:
        ...
        if process_output_dim:
            if action_dim is None:
                action_dim = getattr(env_config, "action_dim", None) if env_config else None
            if action_dim is None:
                raise ValueError("action_dim is required when process_output_dim=True")
            self.module_config_dict = self._process_module_config(
                self.module_config_dict, action_dim
            )
    ...
    def _process_module_config(self, module_config_dict, action_dim):
        """Replace ``"action_dim"`` sentinels with the actual action count.

        Args:
            module_config_dict: Module config containing an ``output_dim`` list.
            action_dim: Number of action dimensions to substitute.
        ...
        for idx, output_dim in enumerate(output_dim_list):
            if output_dim == "action_dim":
                module_config_dict["output_dim"][idx] = action_dim
